...
trait Session
{
    ...
    {    
         ...
         }else if($this->hashBD['session_last_update'] < date("Y-m-d H:i:s")){
              throw new \Exception("token de sessao expirado!", 401);

         }else if($this->hashSession != $this->hashBD['session_key']) {
    ...
    private function updateExpire()
    {
        ...
        $hashDate  = new \DateTime($this->hashBD['session_last_update']);
